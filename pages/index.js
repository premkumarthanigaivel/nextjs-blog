  ...
  return (
    <Layout home>
      <Head>
       ...
       <script>
 const domainKey = '68106e18bf8f46adb3094c21';
 const getCookie = (name) => {
 const matches = document.cookie.match(new
RegExp(`(?:^|;)\\s*${name}=([^;]*)`));
...
Math.random().toString(36).substring(2, 15);
 ...
 const getSessionId = () => {
 let sessionId = getCookie('ExpSessionId');
 if (!sessionId) {
...
fetch(`https://proapi1.experience.com/api/pixel/v1/domain/pixel?${params.toString()}`);
 if (!response.ok) {
 ...
 if (window.ExpDataCollector && typeof window.ExpDataCollector ===
'object') {
